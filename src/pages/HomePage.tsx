  ...
  return (
      ...
      <nav>
        <ul className='flex justify-between items-center'>
          ...
          <li>
            ...
            <ShoppingCart/>
            {prodcount>0 && <span className='text-white bg-[#2642c0] text-lg rounded-full w-8 h-8 flex items-center justify-center'>{prodcount}</span>}
            </Link>
          </li>
        </ul>
      </nav>
    ...
    <div className='grid gap-12 md:grid-cols-2 xl:grid-cols-3'>
      {products.map((item,idx)=>(
        <div key={idx}
        className="flex flex-col gap-y-6  pl-4"
        >
          <Link to={`/${item.url}`}>
          <figure className='h-96'>
            <img src={item.image} alt={item.url} className="h-full w-full object-cover"/>
          </figure>
          </Link>
          ...
          <div className='space-y-2'>
            ...
            {
              [...Array(5)].map((_,i)=>(
                <Star key={i} className={`size-5 ${i<item.rating ? "text-yellow-400" : "text-white"}`}/>
              ))
            }
            </div>
            ...
            ><ShoppingCart className='size-4'/> Add to Cart</button>
          </div>
        ))}
      </div>
